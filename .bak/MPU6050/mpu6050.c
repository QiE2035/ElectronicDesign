...
// double q[0] = 1, q[1] = 0, q[2] = 0, q[3] = 0;  // 四元数的元素，代表估计方向
double exInt = 0, eyInt = 0, ezInt = 0; // 按比例缩小积分误差

double q[4] = {1, 0, 0, 0};

// double Yaw, Pitch, Roll; // 偏航角，俯仰角，翻滚角

void MPU6050_Quaternion(MPU6050_t *DataStruct)
{
    double norm;
    double vx, vy, vz;
    double ex, ey, ez;
    ...
    DataStruct->QuaternionX = asin(-2 * q[1] * q[3] + 2 * q[0] * q[2]) * 57.3;                                                      // pitch ,转换为度数
    DataStruct->QuaternionY = atan2(2 * q[2] * q[3] + 2 * q[0] * q[1], -2 * q[1] * q[1] - 2 * q[2] * q[2] + 1) * 57.3;              // rollv
    DataStruct->QuaternionZ = atan2(2 * (q[1] * q[2] + q[0] * q[3]), q[0] * q[0] + q[1] * q[1] - q[2] * q[2] - q[3] * q[3]) * 57.3; // 此处没有价值，注掉
} */
