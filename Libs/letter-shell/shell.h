...
#define     SHELL_EXPORT_CMD(cmd, func, desc)                               \
            const char shellCmd_##cmd[] = #cmd;                             \
            const char shellDesc_##cmd[] = desc;                            \
            const SHELL_CommandTypeDef                                      \
            shellCommand_##cmd SECTION("shellCommand") =                    \
            {                                                               \
                shellCmd_##cmd,                                             \
                (int (*)())func,                                            \
                shellDesc_##cmd,                                            \
...
#define     SHELL_EXPORT_CMD_EX(cmd, func, desc, help)                      \
            const char shellCmd_##cmd[] = #cmd;                             \
            const char shellDesc_##cmd[] = desc;                            \
            const char shellHelp_##cmd[] = help;                            \
            const SHELL_CommandTypeDef                                      \
            shellCommand_##cmd SECTION("shellCommand") =                    \
            {                                                               \
                shellCmd_##cmd,                                             \
...
#define     SHELL_EXPORT_CMD_EX(cmd, func, desc, help)                      \
    ...
    #define SHELL_EXPORT_VAR(var, variable, desc, type)                     \
            const char shellVar_##var[] = #var;                             \
            const char shellDesc_##var[] = desc;                            \
